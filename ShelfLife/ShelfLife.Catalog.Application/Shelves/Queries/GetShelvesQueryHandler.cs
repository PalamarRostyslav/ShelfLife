...
{
    public class GetShelvesQueryHandler : IRequestHandler<GetShelvesQuery, List<ShelfDto>>
    {
        private readonly ICatalogDbContext _db;

        public GetShelvesQueryHandler(ICatalogDbContext db) => _db = db;

        public async Task<List<ShelfDto>> Handle(GetShelvesQuery request, CancellationToken cancellationToken)
        {
